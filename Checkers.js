...
var tdNum3, pieceEatenByKingID, pieceEatenByKingRow, pieceEatenByKingCol, blackPawnCount = 12, redPawnCount = 12, blackKingCount = 0, redKingCount = 0; //eating
...
function BuildingTheBoard() {
    ...
    //console.log(CountPoints(AllNum, blackKingCount, redKingCount));
}
        ...
        if (clickCounter % 2 == 0) {
            ...
            if (FirstCheckKingMove(AllNum, firstClickRow, firstClickCol, secondClickRow, secondClickCol) == 1) {
                ...
                if (AllNum[pieceEatenByKingRow][pieceEatenByKingCol] == 2)
                    redPawnCount--;
                if (AllNum[pieceEatenByKingRow][pieceEatenByKingCol] == 4)
                    redKingCount--;
        ...
        if (clickCounter % 2 == 0) {
            ...
            if (SecondCheckKingMove(AllNum, firstClickRow, firstClickCol, secondClickRow, secondClickCol) == 1) {
                ...
                if (AllNum[pieceEatenByKingRow][pieceEatenByKingCol] == 1)
                    blackPawnCount--;
                if (AllNum[pieceEatenByKingRow][pieceEatenByKingCol] == 3)
                    blackKingCount--;
                document.getElementById(pieceEatenByKingID).innerHTML = "<img src='BlackTile.png' width='" + boardSize + "' height=' " + boardSize + "'/>";
                AllNum[pieceEatenByKingRow][pieceEatenByKingCol] = 0;
                CheckWin();
...
function CheckWin() {
    if (blackPawnCount == 0 || redPawnCount == 0) {
        ...
        else //if black didn't lose then we can be sure red lost because of line 457
...
function CountPoints(b, bkc, rkc) {
    var boardPoints = 0;
    boardPoints += bkc * kingValue;
    boardPoints -= rkc * kingValue;
    for (var x = 0; x < 8; x++) {
        for (var y = 0; y < 8; y++) {
            switch (x) {
                case 0:
...
function ComputerPlay() {
    if (turns % 2 != 0) {
        var comparePoints = 100, bestfirstRow, bestfirstCol, bestsecondRow, bestsecondCol, bestmiddleRow, bestmiddleCol;
        var tempBlackKingCount, tempRedKingCount, tempEatenRow, tempEatenCol;
        ...
        for (var x = 0; x < 8; x++) {
            for (var y = 0; y < 8; y++) {
                tempArray = JSON.parse(JSON.stringify(AllNum)), tempBlackKingCount = blackKingCount, tempRedKingCount = redKingCount;
                ...
                if (tempArray[firstClickRow][firstClickCol] == 2) {
                    ...
                    if (SecondCheckMove(tempArray, firstClickRow, firstClickCol, secondClickRow, secondClickCol) == 1) {
                        ...
                        if (CountPoints(tempArray, tempBlackKingCount, tempRedKingCount) < comparePoints) {
                            comparePoints = CountPoints(tempArray, tempBlackKingCount, tempRedKingCount);
                    ...
                    for (secondClickRow = firstClickRow; secondClickRow < 8; secondClickRow++) {
                        for (secondClickCol = firstClickCol; secondClickCol < 8; secondClickCol++) {
                            ...
                            if (SecondCheckKingMove(tempArray, firstClickRow, firstClickCol, secondClickRow, secondClickCol) == 1) {
                                ...
                                tempArray[firstClickRow][firstClickCol] = 0;
                                //removes eaten piece
                                if (tempArray[pieceEatenByKingRow][pieceEatenByKingCol] == 3)
                                    tempblackKingCount--;
        ...
        if (bestfirstRow == null || bestfirstCol == null || bestsecondRow == null || bestsecondCol == null) {
            ...
            if (bestmiddleRow != null && bestmiddleCol != null) {
                ...
                else if (AllNum[bestmiddleRow][bestmiddleCol] == 3)
                    blackKingCount--;
                document.getElementById(tdNum3).innerHTML = "<img src='BlackTile.png' width='" + boardSize + "' height=' " + boardSize + "'/>";
